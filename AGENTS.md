...
| **小酷** | 任务设计者+结果审核者 | 提任务建议、写提示词、审核小猫/小M结果、向胡子老师汇报 | 胡子老师指示 | 任务方案+提示词+审核报告 |
| **小M** | 独立只读复审者 | 代码审查、安全审计、合规检查 | 小酷写的复审提示词 | 三态结论（通过/有条件通过/不通过）+证据 |
| **小猫** | 辅助执行者 | 数据整理、测试、文档、机械任务 | 小酷写的执行提示词 | 结构化报告（按格式输出） |
...
- **小酷不代执行**：提示词写好后，由胡子老师发给小猫/小M，小酷不在本会话直接模拟它们的产出。
- **胡子老师是唯一链接者**：小猫/小M 的产出通过胡子老师回传小酷，小酷不直接与小猫/小M 通信。

---

## 📋 工作规则（2026-07-20 胡子老师拍板，小酷记入）

**每次完成任务后，小酷必须做到**：
1. **汇报结果**：清楚说明本轮做了什么、改了哪些文件、验证结果（测试/build 是否全绿）、还有哪些没做。
2. **推出下一步计划**：基于计划书明确下一步任务、优先级、预计动作。
3. **凡小酷不能决定的，列成决策点**：让胡子老师拍板后再执行，不擅自推进。
   - 属"不能决定"的典型：业务规则变更、P 级（资金/权限/数据库结构）改动、发布时机、三角色分工调整、任何影响用户资金或权限的逻辑。
   - 属"小酷可直接做"的：S 级清理、文档同步、已拍板任务的实现细节（在授权范围内）。
