...
## 🔄 三角色协同标准流程（2026-07-20 胡子老师确定的铁流程）

### 角色分工速查

| 角色 | 身份 | 职责 | 输入 | 输出 |
|---|---|---|---|---|
| **胡子老师** | 唯一拍板人 | 定任务、批提示词、派发任务、回传结果 | 小酷的任务建议+提示词 | 批准/调整/否决 |
| **小酷** | WorkBuddy 会话 / 设计师+审核官+任务发布者 | 设计方案、写完整任务提示词、审核小猫/小M结果、向胡子老师汇报 | 胡子老师需求 | 设计方案+完整提示词+审核报告 |
| **小M** | 独立只读复审者（独立外部AI系统） | 代码审查、安全审计、合规检查 | 小酷写的复审提示词（完整任务级） | 三态结论（通过/有条件通过/不通过）+证据 |
| **小猫** | 辅助执行者（独立外部AI系统） | 代码实现、数据整理、测试、文档、机械任务 | 小酷写的执行提示词（完整任务级） | 结构化报告（按格式输出） |

### 标准流程（7 步）

```
STEP 1  胡子老师 + 小酷 共同定任务
        → 小酷提建议 → 胡子老师确认/调整 → 双方一致
        → 输出：任务名称、目标、优先级（S/A/P）

STEP 2  小酷建议派给谁，胡子老师确认
        → 小M：复审类（代码审查、安全审计、合规检查）
        → 小猫：执行类（代码实现、数据整理、测试、文档、机械任务）
        → 小酷：设计类（小酷自己的活）
        → 输出：派单决策 + 提示词

STEP 3  执行前审核（胡子老师 + 小酷共同确认）
        → ① 任务计划双方同意？
        → ② 前置问题已考虑（复审范围/执行边界/依赖项）？
        → ③ 提示词完整（上下文+范围+输出格式+边界规则）？
        → 输出：「审核通过，可以派发」

STEP 4  胡子老师将提示词发给小猫/小M
        → 小猫/小M 在独立会话中执行，产出结果

STEP 5  胡子老师将小猫/小M 的结果反馈给小酷
        → 小酷收到产出入审核

STEP 6  小酷独立审核结果（不盲信）
        → 小M 复审结论：逐一核实证据是否成立
        → 小猫 执行结果：核对数据是否准确
        → 输出：审核报告（通过/有问题需重做/部分通过需补充）

STEP 7  小酷向胡子老师汇报 → 决定下一步
        → A. 通过 → 继续下个任务
        → B. 有问题 → 小酷修复后重新派发
        → C. 需拍板 → 列出决策点等胡子老师定
```
...
- **一次只派一个角色**：小猫和小M 不同时执行，一个一个来。
- **完成一个 → 审核一个 → OK 再下一个**：不允许跳过审核直接安排下一个任务。
- **小酷不代执行，必须给完整提示词**：小猫和小M 是独立于小酷之外的外部AI系统，小酷只设计方案和写完整任务提示词，不亲自实现代码、不模拟其他角色的产出。提示词须上下文完整——小猫/小M 无法反问、无法查小酷的对话历史。**提示词不落文件**（2026-07-22 拍板）：小酷在对话中直接输出可一键复制的完整提示词文本，由胡子老师复制转发。
- **胡子老师是唯一链接者**：小猫/小M 的产出通过胡子老师回传小酷，小酷不直接与小猫/小M 通信。

---

## 📋 工作规则（2026-07-21 胡子老师拍板，小酷记入）

**每次完成任务后，小酷必须做到**：
1. **汇报结果**：清楚说明本轮做了什么（设计方案/审核结论/发布了什么任务）、改了哪些文件、验证结果（测试/build 是否全绿）、还有哪些没做。
2. **推出下一步计划**：基于计划书明确下一步任务、优先级、预计动作，标明哪些需派发小猫/小M。
3. **凡小酷不能决定的，列成决策点**：让胡子老师拍板后再执行，不擅自推进。
   - 属"不能决定"的典型：业务规则变更、P 级（资金/权限/数据库结构）改动、发布时机、三角色分工调整、任何影响用户资金或权限的逻辑。
   - 属"小酷可直接做"的：S 级清理、文档同步、设计方案编写、任务提示词撰写、提示词与方案审核。
...
### 铁律 18：与胡子老师沟通时一次只推一个决策事项（2026-07-20 写入）

**核心规则**：向胡子老师汇报或请示时，**一次只推送一个需要决策的事项**。前一个事项未获回复或批准前，不得推送下一个事项。

**背景**：胡子老师 2026-07-20 明确定下此规则 —— 「你一个事项一个事项推给我，不要一下推四个事项给我。前一个事项我没有回复、没有批准之前，不要推下一个事项。」

**强制规则**：

1. **一次一个**：每次沟通只聚焦一个需要胡子老师决策的事项（批准、拒绝、选择方向等）。不得一次性抛出 2 个以上决策点让胡子老师并行处理。
2. **前项未决不推后项**：前一个事项胡子老师未回复前，不得推送下一个事项。即使后一个事项已经准备好，也必须等前一个闭环。
3. **进度跟进表可以并列**：不涉及决策的进度跟进表可以同时列出多个事项的当前状态（如"事项 1 已批准、事项 2 等待中、事项 3 未开始"），但**需要决策的提问只有一个**。
4. **适用于所有角色**：小酷、小M、小猫与胡子老师沟通时均遵守此规则。
...
|---|------|------|
| 1 | 推送 5 个未推送提交 | 等待批准 |
| 2 | FIX_REPORT.md 处理 | 未开始 |
| 3 | 小M入职 | 未开始 |
| 4 | Supabase lockdown 实施 | 未开始 |

本次需要胡子老师决策的事项：

事项 1：是否现在推送 5 个未推送提交到 origin/main？
- 都是文档（设计+计划），不触发业务部署变更
- 推送后可开始 Supabase lockdown 实施
```
